jQuery( document ).ready(function() {
  ...
  if ( jQuery( ".portrait-img" ).length ) {
    ...
    jQuery( ".portrait__content img" ).each(function( index ) {
      ...
      jQuery(this).closest('.portrait__image').css('background-image','url(' + currentImg + ')');
    });
  }/*
...
if ( jQuery( ".carte" ).length ) {
		if ( !jQuery( ".carte_flex" ).length ) {
    		jQuery('.carte').wrapAll('<div class="carte_flex"></div>');
			
		}
		
		jQuery('.carte__content p img').each(function() {
			jQuery(this).prependTo(jQuery(this).closest('.carte__content'));
		});
		jQuery('.carte__content p').each(function() {
